...
program
	...
	.option('-m, --module [value]', 'Module from which to import the type. "" to use global type.')
	.option('-t, --type [value]', 'The type to use. Must accept 4 generic type arguments: in, uniform, out, type')
	.option('-g, --generator [value]', 'Custom generator to use. Must be a js file with the default export being a function with the following signature:\n'
...
function handleTypes(filePath: string, typings: Result) {
	...
	try {
		const output = getTypeScriptDTS(typings, program.type, 'boolean' == typeof program.module ? false : program.module)
...
const handler = program.generator ? require(program.generator).default : handleTypes
