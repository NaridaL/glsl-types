...
export function getTypings(shaderSource: string) {
  ...
  for (const node of r) {
    if (node instanceof InitDeclaratorList || node instanceof InterfaceBlock) {
      ...
      const storage =
        node instanceof InitDeclaratorList ? node.type.storage : node.storage
      if (!storage) continue
      ...
      if ("UNIFORM" == storage_type) {
        ...
        "IN" == storage_type ||
        ("VARYING" == storage_type && !wroteGLPosition)
      ) {
        interface2 = "in_"
      } else if (
        "OUT" == storage_type ||
      ...
      if (node instanceof InitDeclaratorList) {
        ...
        for (const d of node.members) {
          ...
          for (const ddn in ds) {
            result[interface2][
              (node.instanceName ? node.name.value + "." : "") + ddn
            ] = ds[ddn]
          }
        }
      }
...
export function getTypeScriptDTS(
  { in_, uniform, out, type }: Result,
  shaderType = "ShaderSource",
  module: string | false = "tsgl",
) {
