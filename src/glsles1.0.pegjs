...
start
  = external_statement_list {
	  return {globalDeclarations, wroteGLPosition}
  }
...
type_qualifier
	= "invariant" _ storage_qualifier
	/ "invariant" _ interpolation_qualifier _ storage_qualifier
	/ storage_qualifier
	/ layout_qualifier
	/ layout_qualifier _? storage_qualifier
	/ interpolation_qualifier _ storage_qualifier
...
layout_qualifier
	= "layout" _? "(" layout_qualifier_id_list ")"
layout_qualifier_id_list
	= layout_qualifier_id
	/ layout_qualifier_id _? "," _? layout_qualifier_id_list
layout_qualifier_id
	= identifier
	/ identifier _? "=" _? int_constant
	/ identifier _? "=" _? int_constant
...
assignment_expression
  = variable:unary_expression _?
    operator:("=" / "*=" / "/=" / "%=" /
              "+=" / "-=" / "<<=" / ">>=" /
              "&=" / "^=" / "|=") _?
    expression:assignment_expression {
	  if ('identifier' == variable.type && 'gl_Position' == variable.name) {
		  wroteGLPosition = true
	  }
      return new node({
        type: "binary",
        operator: new node({
